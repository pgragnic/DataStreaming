...
spark = SparkSession.builder.appName("risk-assesment").getOrCreate()
...
redisEncodedStreamingDF = spark.sql("select key, zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
redisDecodedStreamingDF = redisEncodedStreamingDF \
            .withColumn("customer", unbase64(redisEncodedStreamingDF.encodedCustomer).cast("string"))
...
customerRiskStreamingDF = spark.sql("select customer, score from CustomerRisk where customer is not null and score is not null")
...
joinedDF.selectExpr("CAST(customer AS STRING) AS key", "to_json(struct(*)) AS value")\
    ...
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("topic", "risk-information")\
    .option("checkpointLocation", "/tmp/kafkacheckpoint/risk-information/")\
    .start()\
    .awaitTermination()
